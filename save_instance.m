% batch processing of *.rcp files to (parsed) *.mat files as an input for simulation
...
% example: save_instance('Patterson')
...
for i=1:size(files,1) % iterate through all files in given directory
    ...
    [PDM, constr, num_r_resources, num_modes] = parse_instance(fullfile(files(i,1).folder,files(i,1).name),j); % parse all files for all simulation types
    ...
    if (j == 1) % save DSM only once per file
        DSM = parse_instance(fullfile(files(i,1).folder,files(i,1).name),0); % parse only DSM
